...
for(i in 1: length(folders1)){
  ...
  FPDEstDist <- vector()
  for(i in 1:100){
    x <- sample(simtree$tip.label, 20) #samples a random set of tips
    x <- x
  ...
  FPDWilcox <- wilcox.test(FPDSimDist, FPDEstDist, paired = T)
  FPDttest <- t.test(FPDSimDist, FPDEstDist, paired = T)
...
setwd("results/FPD/Random/Sample20")
save(M1FPDtest, file = "M1FPDpvalueRd.Rdata")
save(M1FPDVvalue, file = "M1FPDVvalueRd.Rdata")

png("Model1_FPDRd_p.values_hist.png")
...
setwd("results/FPD/Random/Sample20")
save(M2FPDtest, file = "M2FPDpvalueRd.Rdata")
save(M2FPDVvalue, file = "M2FPDVvalueRd.Rdata")  

png("Model2_FPDRd_p.values_hist.png")
...
setwd("results/FPD/Random/Sample20")
save(M3FPDtest, file = "M3FPDpvalueRn.Rdata")
save(M3FPDVvalue, file = "M3FPDVvalueRn.Rdata")

png("Model3_FPDRn_p.values_hist.png")
...
setwd("results/FPD/Random/Sample20")
save(M4FPDtest, file = "M4FPDpvalueRn.Rdata")
save(M4FPDVvalue, file = "M4FPDVvalueRn.Rdata")

png("Model4_FPDRn_p.values_hist.png")
...
plot(M1simFPDstn, M1estFPDstn, xlab = "Simulated FPD scores", ylab = "Reconstructed FPD scores", xlim = range(.0, .36), ylim = range(0.0, .36), col = rgb(0, 0, 0, .2))
dev.off()
png("Model2_FPD_scores.png")
plot(M2simFPDstn, M2estFPDstn, xlab = "Simulated FPD scores", ylab = "Reconstructed FPD scores", xlim = range(.0, .36), ylim = range(0.0, .36), col = rgb(0, 0, 0, .2))
dev.off()
png("Model3_FPD_scores.png")
plot(M3simFPDstn, M3estFPDstn, xlab = "Simulated FPD scores", ylab = "Reconstructed FPD scores", xlim = range(.0, .36), ylim = range(0.0, .36), col = rgb(0, 0, 0, .2))
dev.off()
png("Model4_FPD_scores.png")
plot(M4simFPDstn, M4estFPDstn, xlab = "Simulated FPD scores", ylab = "Reconstructed FPD scores", xlim = range(.0, .36), ylim = range(0.0, .36), col = rgb(0, 0, 0, .2))
dev.off()
